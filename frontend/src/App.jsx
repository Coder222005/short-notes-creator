...
export default function App() {
  ...
  const [theme, setTheme] = useState(() => {
    const saved = localStorage.getItem('study_notebook_theme');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('study_notebook_theme', theme);
  }, [theme]);
        ...
        <header className="main-header">
          <div className="header-title-container">
            <span className="header-title">
              {activeNotebook ? activeNotebook.name : "StudyNotebook"}
            </span>
          </div>
          <div className="header-actions-group" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <button className="header-model-badge" onClick={() => setIsSettingsOpen(true)}>
              <span style={{
                ...
                height: '7px',
                borderRadius: '50%',
                backgroundColor: settings.provider === 'fallback' ? 'var(--quiz-avg)' : 'var(--quiz-correct)'
              }}></span>
